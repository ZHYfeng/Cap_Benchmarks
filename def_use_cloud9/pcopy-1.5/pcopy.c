...
void *writer(void *foo) {
  ...
  while (bp = q_pop(write_q)) {
    if (x_pwrite64(bp->pos, bp->buf, bp->len) != bp->len) {
      ...
      fprintf(stderr, "pcopy: %s: Error: Write of %d bytes failed at "
#if SIZEOF_LONG == 8
                      "%ld"
#else
                      "%lld"
#endif
                      ": %s\n",
              dstdev, bp->len, bp->pos, strerror(errno));
      return 1;
    }
...
#if SIZEOF_LONG == 8
          printf("\rCopied: %ld MB (%ld MB/sec)     ", written / 1024 / 1024,
                 mbs);
#else
          printf("\rCopied: %lld MB (%lld MB/sec)     ", written / 1024 / 1024,
                 mbs);
#endif
          fflush(stdout);
          last = now;
...
Loop:
  ...
  while ((len = x_pread64(pos, bp->buf, size)) < 0 && errno == EIO) {
    ...
    if (size > 512) {
      ...
      fprintf(stderr, "pcopy: %s: Warning: Problems reading %d bytes at "
#if SIZEOF_LONG == 8
                      "%ld"
#else
                      "%lld"
      ...
      fprintf(stderr, "pcopy: %s: Error: Read of %d bytes at "
#if SIZEOF_LONG == 8
                      "%ld"
#else
                      "%lld"
