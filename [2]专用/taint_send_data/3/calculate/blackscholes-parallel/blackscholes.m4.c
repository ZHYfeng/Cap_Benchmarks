...
#pragma omp parallel for private(i, price, priceDelta)
        ...
        for (i=start; i<end; i++) {
             ...
             * Black & Scholes's equation.
             */
             sptprice[i]=sptprice[i];
             make_taint(&sptprice[i]);
             volatility[i]=volatility[i];
             make_taint(&volatility[i]);
                rate[i]=rate[i];
             make_taint(&rate[i]);
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
   ...
   if (argc != 4)
    ...
    for ( loopnum = 0; loopnum < numOptions; ++ loopnum )
    {
        ...
        data[loopnum].s=data[loopnum].s;
        make_taint(&data[loopnum].s);
        data[loopnum].strike=data[loopnum].strike;
        make_taint(&data[loopnum].strike);
        data[loopnum].r=data[loopnum].r;
